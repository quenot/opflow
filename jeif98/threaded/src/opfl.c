...
int xopflow(SIMAGE *sim, VIMAGE *vim, DPCONTROL *dpcontrol, int n)
{
    ...
    int ff,lf,mx,my,mw,sum,wx,sc,ar,dni,bsi,smf,fail = 0;
    ...
    #pragma omp parallel for num_threads(16) private(dd,gg,bb,ky,l,wm,wp,km,kp)
    for (k = 0; k < ne; k++) {
        if (fail) continue;
        ky = k*nl+wd+ql;
        wm = l = (wl+wr)/2;
        ...
        km = imax(0,ky-mw);
        kp = imin(my,ky+mw);
        if ((altens2((void ***) &dd,0,wx,-na-1,na+1,sizeof(fdata)) == FAILURE)
        ...
        if ((frtens2((void ***) &dd,0,wx,-na-1,na+1,sizeof(fdata)) == FAILURE)
